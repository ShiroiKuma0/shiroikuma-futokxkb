...
Last shipped: `kxkb: Multiling-style "topBar" — static candidates shown when nothing is typed` (commit `432f70487`). Fixed entries (emoji/snippets, `A…B` bracket-pairs with caret-between, `[Paste]`, `{{date` stamps) shown in the suggestion bar in its empty/neutral state (between words, after Enter, after picking). Global list editable in Settings → Typing → "Suggestion bar candidates"; a layout's own `topBar:` overrides it. Engine in `GeneralIME.showEmptyOrTopBar()` (centralised so every empty-strip path shows it — not just `setNeutralSuggestionStrip`, which caused flashing in the first cut) + `engine/general/TopBar.kt` parser + pick interception in `onEventInternal`. 7 files (2 new), no native change. Documented in full in futo-keyboard-build §"Implemented features".
